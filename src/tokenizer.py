...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # downsampling
        hs = [self.conv_in(x)]
        # print("log conv_in", hs[0].shape)
        for i_level in range(self.num_resolutions):
            for i_block in range(self.num_res_blocks):
                h = self.down[i_level].block[i_block](hs[-1], temb)
                if len(self.down[i_level].attn) > 0:
                    h = self.down[i_level].attn[i_block](h)
                # print("log new shape", i_level, i_block, h.shape)
                hs.append(h)
            if i_level != self.num_resolutions - 1:
                hs.append(self.down[i_level].downsample(hs[-1]))
                # print("log last new shape", i_level, hs[-1].shape)
...
class AutoencoderKLEncoder(nn.Module):
    def __init__(self, embed_dim, ch_mult, use_variational=True, ckpt_path=None, img_size = 256):
        super().__init__()
        # print("log", ch_mult, type(ch_mult))
